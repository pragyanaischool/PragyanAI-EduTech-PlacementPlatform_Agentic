...
from sqlalchemy import create_engine, Column, Integer, String, Float, Boolean, Text, ForeignKey, DateTime, event
...
# Immediately bind metadata schemas to avoid missing table OperationalErrors
...
def authenticate_user(username: str, password: str):
    ...
    session = SessionLocal()
    try:
        # Seed default admin if user table was just created
        if session.query(UserModel).count() == 0:
