...
def df_to_markdown_table(df: pd.DataFrame) -> str:
    """Renders a pandas DataFrame into a standard Markdown table without requiring `tabulate`."""
    if df.empty:
...
def handle_placement_chat(query: str, user_role: str, user_context: dict = None) -> str:
    ...
    if groq_api_key:
        try:
            ...
            table_markdown = df_to_markdown_table(result_df)
            insight_prompt = f"""
...
"""
            ...
            pass  # Fallback to local analytical engine if API encounters limits or network issues
    ...
    if any(k in q_lower for k in ["list of student", "show student", "view student", "students list", "all student"]):
        ...
        table_md = df_to_markdown_table(sample_df)
        return (
    ...
    if any(k in q_lower for k in ["all compan", "list of compan", "show compan", "hiring partner", "recruiters"]):
        ...
        table_comp_md = df_to_markdown_table(sample_comp)
        return (
