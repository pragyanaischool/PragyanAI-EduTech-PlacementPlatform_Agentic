...
selections_df = st.session_state.get("drive_selections", pd.DataFrame())
...
interviewing_cnt = len(stages_df[~stages_df["Current_Round"].str.contains("Selected|Offer", case=False, na=False)])
...
with tab_postdrive:
    ...
    with col_f1:
        filter_dept_cohort = st.multiselect("Filter Candidate Pool by Department:", students_df["Dept"].unique(), default=eligible_depts if all(d in students_df["Dept"].unique() for d in eligible_depts) else students_df["Dept"].unique())
    with col_f2:
...
with tab_stages:
    ...
    with st.form("stage_progression_form"):
        ...
        with col_p2:
            comp_list = drives_df["Company"].unique().tolist()
...
with tab_analytics:
    ...
    if stages_df.empty:
        ...
        with col_fa1:
            stage_volume = stages_df.groupby("Current_Round")["Student_ID"].count().reset_index()
...
with tab_roster:
    ...
    with col_r1:
        f_dept = st.multiselect("Filter Dept:", students_df["Dept"].unique(), default=students_df["Dept"].unique(), key="ros_dept")
    with col_r2:
        f_status = st.multiselect("Filter Status:", students_df["Status"].unique(), default=students_df["Status"].unique(), key="ros_stat")
    with col_r3:
        f_year = st.multiselect("Graduation Year:", students_df["Grad_Year"].unique(), default=students_df["Grad_Year"].unique(), key="ros_yr")
    ...
    st.dataframe(filtered_roster[display_roster_cols], use_container_width=True, hide_index=True)

    csv_data = filtered_roster[display_roster_cols].to_csv(index=False).encode("utf-8")
    st.download_button(
