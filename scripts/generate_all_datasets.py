...
# ----------------------------------------------------
# 1. TAXONOMIES & DEPARTMENTS
# ----------------------------------------------------
DEPTS = {
    "CSE": {"weight": 0.22, "skills": ["Python", "Java", "C++", "FastAPI", "React", "Docker", "Kubernetes", "SQL", "Redis", "Kafka", "Data Structures", "System Design", "Go", "GraphQL", "gRPC"]},
    ...
    "AIDS": {"weight": 0.14, "skills": ["Python", "R", "SQL", "Pandas", "PowerBI", "Tableau", "PySpark", "Machine Learning", "Data Mining", "Statistics", "BigQuery", "Snowflake", "dbt"]},
    "ISE": {"weight": 0.12, "skills": ["Java", "Spring Boot", "Go", "AWS", "GCP", "Microservices", "REST APIs", "PostgreSQL", "CI/CD", "Linux", "Algorithms", "Terraform", "Node.js"]},
    "ECE": {"weight": 0.14, "skills": ["Embedded C", "C++", "Verilog", "VHDL", "ARM Cortex", "RTOS", "Linux Device Drivers", "PCB Design", "SPI", "I2C", "DSP", "SystemVerilog", "UVM"]},
...
DOMAINS_AND_COMPANIES = {
    ...
    "Aerospace, Energy & Industrial": [
        "Boeing", "Airbus", "General Electric", "Siemens", "Honeywell",
...
# ----------------------------------------------------
# 2. GENERATE COMPANIES, DRIVES & JDs
# ----------------------------------------------------
companies_master = []
drives_list = []
jds_list = []
    ...
    for comp in comp_names:
        ...
        for role in selected_roles:
            ...
            drives_list.append({
                ...
                "Description": f"Hiring for {role} in {domain}. Involves core product engineering and production delivery.",
...
# ----------------------------------------------------
# 3. GENERATE 1,500 STUDENTS & CV CORPUS
# ----------------------------------------------------
FIRST_NAMES = ["Aarav", "Priya", "Rohan", "Sneha", "Ananya", "Kiran", "Vikram", "Neha", "Aditya", "Pooja", "Rahul", "Divya", "Sanjay", "Meera", "Varun", "Kavya", "Siddharth", "Ishita", "Arjun", "Ritu", "Gaurav", "Swati", "Naveen", "Tanvi", "Akash", "Bhavya", "Harish", "Shruti", "Manish", "Deepika"]
LAST_NAMES = ["Sharma", "Patel", "Iyer", "Reddy", "Roy", "Varma", "Sen", "Nair", "Kulkarni", "Hegde", "Deshmukh", "Bhat", "Rao", "Joshi", "Gupta", "Agarwal", "Mishra", "Choudhury", "Pillai", "Menon"]
...
for i in range(1, TOTAL_STUDENTS + 1):
    ...
    projects = f"Engineering project in {dept}: Developed prototype using {skills[0]} and {skills[1]}."
    ...
    if is_placed:
        ...
        selections_list.append({
            "Drive_ID": chosen_drive["Drive_ID"], "Company": comp_name, "Student_ID": stu_id,
            "Student_Name": name, "Dept": dept, "Attended": True,
            "Selection_Status": "Selected", "Offered_Role": role_name, "Offered_CTC_LPA": package_lpa
        })
        stages_list.append({
            "Stage_ID": f"STG-{i:04d}", "Student_ID": stu_id, "Student_Name": name, "Dept": dept,
            "Company": comp_name, "Role": role_name, "Current_Round": "Offer Extended (Selected)",
            "Next_Round_Date": "Completed", "Mode_Location": f"{comp_name} Corporate Campus"
        })
        
        if len(experiences_list) < 150:
            experiences_list.append({
                "Exp_ID": f"EXP-{len(experiences_list) + 1001}",
                "Student_ID": stu_id, "Student_Name": name, "Dept": dept,
                "Company": comp_name, "Role": role_name,
                "Rounds_Faced": "Round 1: Technical Screening, Round 2: Architecture & Live Coding, Round 3: Leadership/HR",
                "Skills_Excelled": f"{skills[0]}, {skills[1]}, Problem Solving Under Pressure",
                "Challenges_Faced": "Complex questions on edge failure handling and scaling trade-offs.",
                "Advice_To_Crack": "Focus on fundamentals, dry run edge cases, and communicate assumptions clearly.",
        ...
        if random.random() < 0.30:
            ...
            stages_list.append({
                "Stage_ID": f"STG-{i:04d}", "Student_ID": stu_id, "Student_Name": name, "Dept": dept,
                "Company": random_drive["Company"], "Role": random_drive["Role"],
    ...
    students_list.append({
        "ID": stu_id, "Name": name, "Dept": dept, "Grad_Year": grad_year, "CGPA": cgpa,
        "Skills": skills_str, "Projects": projects, "Experience": experience,
        "Linkedin": f"https://linkedin.com/in/{fname.lower()}-{lname.lower()}-{i}",
        "Github": f"https://github.com/{fname.lower()}{i}",
        "Dream_Roles": dream_roles, "Dream_Companies": dream_comps,
        "Salary_Expected_LPA": salary_exp, "Status": status,
        "Company": comp_name, "Role": role_name, "Package_LPA": package_lpa
    })

    cv_corpus_list.append({
        "Student_ID": stu_id, "Full_Name": name, "Dept": dept,
...
# ----------------------------------------------------
# 4. GENERATE RECRUITER FEEDBACK & TRAINING SESSIONS
# ----------------------------------------------------
recruiter_feedback_list = [
    {
        "Company": "Google", "Drive_ID": "DRV-001", "Evaluator": "Staff Engineering Manager",
        "Dept_Evaluated": "CSE, AIML, ISE", "Overall_Rating": 4.3,
    ...
    {
        "Company": "Qualcomm", "Drive_ID": "DRV-025", "Evaluator": "Senior Director of Silicon Software",
        "Dept_Evaluated": "ECE, EEE", "Overall_Rating": 3.9,
    ...
    {
        "Company": "Synthlinx AI", "Drive_ID": "DRV-019", "Evaluator": "Head of AI Research",
        "Dept_Evaluated": "AIML, AIDS", "Overall_Rating": 4.5,
    ...
    {
        "Company": "Tesla", "Drive_ID": "DRV-038", "Evaluator": "Lead Powertrain Architect",
        "Dept_Evaluated": "MECH, ROBOTICS, EEE", "Overall_Rating": 4.1,
...
training_sessions_list = [
    {
        "Session_ID": "TRN-101", "Type": "Bootcamp",
        "Title": "Generative AI & Agentic Workflows with LangGraph",
        "Target_Depts": "CSE, AIML, AIDS, ISE", "Instructor": "Dr. Sateesh Ambesange (PragyanAI)",
        "Schedule_Date": (datetime.now() + timedelta(days=5)).strftime("%Y-%m-%d"),
        "Timing": "10:00 AM - 04:00 PM", "Mode": "Hybrid", "Location": "Pragyan DeepTech Lab / Zoom",
    ...
    {
        "Session_ID": "TRN-102", "Type": "Workshop",
        "Title": "Bare-Metal ARM Firmware & RTOS Preemption Intensive",
        "Target_Depts": "ECE, EEE, ROBOTICS", "Instructor": "Lead Architect, Qualcomm",
        "Schedule_Date": (datetime.now() + timedelta(days=8)).strftime("%Y-%m-%d"),
        "Timing": "09:30 AM - 01:30 PM", "Mode": "Offline", "Location": "Embedded Systems Lab, Block B",
    ...
    {
        "Session_ID": "TRN-103", "Type": "Masterclass",
        "Title": "Distributed Systems & Raft Consensus in High-Scale FinTech",
        "Target_Depts": "CSE, ISE, AIDS", "Instructor": "Principal Systems Engineer, Amazon",
        "Schedule_Date": (datetime.now() + timedelta(days=12)).strftime("%Y-%m-%d"),
        "Timing": "02:00 PM - 05:00 PM", "Mode": "Online", "Location": "Google Meet",
...
for filename, data in datasets.items():
    ...
    print(f"✅ Generated data/{filename} ({len(data)} records)")

print("\n🚀 All 10 CSV data files have been created in the data/ directory.")
